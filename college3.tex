...
\begin{document}
    ...
    \begin{questions}
	    ...
	    \begin{parts}
	    	...
	    	\begin{proof}
	    		...
	    		\begin{itemize}
	    			...
	    			\]
	    		\end{itemize}
	    		We have now proven that, if $a_n \to \pm\infty$, $a_n \neq 0$ and $b_n$ is bounded, then $a_n + b_n \to \pm \infty$.
	    	\end{proof}
	    	...
	    	\begin{proof}
	    		...
	    		\begin{itemize}
		    		...
		    		\]
			    	  Choose $N$ such that $N > 0$. Then $a_n > 0$ for $n \geq n_0$. In this case it also holds that
		    		\[
			    		\left|\frac{b_n}{a_n}\right| = \frac{|b_n|}{a_n} < \frac{K}{a_n}\,.
		    		\]
			    		Since $\epsilon > 0$ and $K > 0$, we have $\frac{K}{\epsilon} > 0$. Choosing $N = K/\epsilon$ and $n^* = n_0$, we find that for $n\geq n^*$ we have
		    		\[
	    ...
	    \begin{alignat}{3}
	    	...
	    	\label{th:lim:a*b:infinite-} \tag{$\lim_{-\infty}\colon a_nb_n$}	a_n &\to -\infty \,, \enskip b_n > 0 \enskip (b_n < 0) \quad &&\implies \qquad &&a_nb_n \to -\infty \enskip (\infty)\,, \\
	    	\label{th:lim:c/a:infinite} \tag{$\lim_\infty\colon c/a_n$}	a_n &\to \pm\infty \,, \enskip a_n \neq 0\,, \enskip c>0 \quad &&\implies \qquad &&\frac{c}{a_n} \to 0\,, \\
	    ...
	    \begin{parts}
	    	...
	    	\begin{proof}
	    		...
	    		r_n &:= n\,,
	    		\end{align*}
	    		then for $b_n$ we have $a_n = p_n + q_nr_n$. We know $\sqrt n \to \infty$, hence using \ref{th:lim:c/a:infinite} we have $p_n \to 0$. Of course $n \to \infty$ such that $r_n \to \infty$.
	    	...
	    	\begin{proof}
	    		...
	    		\]
	    		But then as $n \to \infty$, using \ref{th:lim:c/a:infinite} and \ref{th:lim:c*a:finite}, we have $\frac{3}{n} \to 0$, such that also $q_n \to 0$ (using the fact that $q_n > 0$ for all $n$). Now $p_n$ is bounded by 1, i.e.
	    		\[
		    		\forall_{n\in\naturals}: |p_n| \leq 1\,.
	    		\]
	    	...
	    	\begin{proof}
	    		...
	    		\end{align*}
	    		We know this converges to 0, because of the following standard limit:
	    		\[
		    		\lim_{n \to \infty} \frac{n^k}{a^n} = 0,
	    		\]
	    		as long as $|a| > 1$ and $k \in \naturals$. 
	    		In our case $a = 1.01$ and $k = 2007$. But then we know $p_n \to 0$. We can use this to explore the behavior of sequence $b_n$ using \ref{th:lim:c/a->0:finite} using the fact that $p_n > 0$ for all $n$;
	    		\[
	    	...
	    	\begin{proof}
	    		...
	    		Using \ref{th:lim:c/a:infinite}, we can conclude that $p_n, q_n, r_n, s_n \text{ and } t_n$ all converge to 0. Using this information, we can explore the behavior of $e_n$:
	    		\[
		    		e_n = \frac{p_n - q_n + r_n}{1 + s_n + t_n} \xrightarrow{n\to\infty} 0\,.
	    		\]
	    	\end{proof}
	    	...
	    	\begin{proof}
	    		...
	    		\begin{align*}	
		    		p_n := \sqrt[n]{n}\,.	
	    		\end{align*}
	    		This is a standard limit, that converges to $1$. We can use this to explore the behavior of $f_n$ using \ref{th:lim:a^p:finite} where $p = 3$;
	    		\[
	    	...
	    	\begin{proof}
	    		...
	    		\[
		    		\lim_{n \to \infty} \frac{n^k}{a^n} = 0\,,
	    		\]
	    		because in both cases $|a| > 1$ and $k \in \naturals$. $q_n$ converges to $0$ using another standard limit:
	    		\[
		    		\lim_{n \to \infty} a^n \to 0 \text{ if } |a| < 1\,,
	    		\]
	    		because here $a = \frac{2}{3} < 1$. Lastly, $s_n$ also converges to $0$ because of \ref{th:lim:c/a:infinite} and the fact that $3^n \xrightarrow{n\to\infty} \infty$.
	    		
	    		Now, we can use all of this information to explore the behavior of $g_n$ using \ref{th:lim:c/a->0:finite};
	    		\[
	    	...
	    	\begin{proof}
	    		...
	    		\end{align*}
	    		$p_n, q_n$ and $r_n$ all converge to $0$, because of the limit theorem \ref{th:lim:c/a:infinite} and the fact that $2n\sqrt n\,, n^2 \sqrt n \xrightarrow{n\to\infty} \infty$. For $q_n$ we can use \ref{th:lim:a^p:finite} such that $\sqrt[6]{n} = n^{\frac{1}{6}} \xrightarrow{n\to\infty} \infty$. We can use this information to explore the behavior of $h_n$ using \ref{th:lim:c/a->0:finite} and the fact that $q_n + r_n > 0$ for all $n \in \naturals_+$;
	    		\[
	    	...
	    	\begin{proof}
	    		...
	    		\begin{align*}	    		
		    		...
		    		r_n &:= \frac{3}{n^8}
	    		\end{align*}
	    		All these sequences converge to $0$, because of the limit theorem \ref{th:lim:c/a:infinite}. We can use this information to explore the behavior of $k_n$:
	    		\[
