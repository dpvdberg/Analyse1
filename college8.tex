    ...
    \begin{questions}
        ...
        \begin{proof}
        	...
        	\begin{itemize}        		
        		...
        		\[
	        		\exists_{\delta > 0} \forall_{x \in D \cap (-\infty, a]} : 0 < a - x < \delta \Rightarrow |f(x) - L| < \epsilon
        		\]
        		Also $x_n \to a$, so
        		\[
        		...
        		Now 
        		\[
	        		\exists_{n_0 \in \naturals}\forall_{n \ge n_0}: 0 < a - x_n < \delta \Rightarrow |f(x_n) - L| < \epsilon
        		\]
        		...
        		This means:
        		\[
	        		\exists_{\epsilon>0} \forall_{\delta > 0} \exists_{x \in D \cap (-\infty, a]} : 0 < a - x < \delta \wedge |f(x) - L| \ge \epsilon
        		\]
        ...
        \begin{proof}
        	...
        	\begin{itemize}        		
        		...
        		Also $x_n \to b$, so
        		\[
	        		\forall_{\delta > 0} \exists_{n_0 \in \naturals} \forall_{n \ge n_0} : 0 < x_n - b < \delta
        		\]
        		Now 
        		\[
	        		\exists_{n_0 \in \naturals}\forall_{n \ge n_0}: 0 < x_n - b < \delta \Rightarrow f(x_n) \ge M
        		\]
        		Since $(x_n)$ was an arbitrary sequence, it now holds that:
        		\[
	        		\forall \text{ sequences } (x_n) \text{ in } D \cap (b, \infty) \text{ with } x_n \to b \text{ holds } f(x_n) \to \infty
        ...
        \begin{parts}
        	...
        	\begin{proof}
        		...
        		\]
        		But since $\forall_{x \in I}: g(x) \neq 0 $, so:
        		\[
	        		\exists_{\bar x \in I} : g(\bar x) < 0 \wedge \exists_{x^* \in I} : g(x^*) > 0
        		\]
        		Because $g(x)$ is continuous, because of the intermediate value theorem on $[\bar x, x^*]$ it holds that:
        		\[
	        		\text{ because } g(\bar x) < 0 < g(x^*) \text{ it holds that } \exists_{c \in (\bar x, x^*)} : g(c) = 0\,. \quad \lightning
        		\]
        	\end{proof}
        	        	
        	\part The (by pointwise arithmetic operations defined) functions 
        	\[
        	...
        	\begin{proof}
        		$f, g$ continuous on $I$ , so:
        		\[
	        		\forall_{a \in I} : \forall \text{ sequences } (x_n) \text{ in } I \text{ with } x_n \to a \Rightarrow f(x_n) \to f(a)
        		\]
        		and also:
        		\[
	        		\forall_{a \in I} : \forall \text{ sequences } (y_n) \text{ in } I \text{ with } y_n \to a \Rightarrow g(y_n) \to g(a)
        		\]
        		Now let $a \in I$ be fixed and arbitrary. Let $x_n$ be an arbitrary sequence in $I$ with $x_n \to a$. Now, for sequences we know:
        		\begin{align*}
