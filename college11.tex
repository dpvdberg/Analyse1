    ...
    \begin{questions}
        ...
        \nquestion{3}
			...
			\begin{proof}
				...
				\begin{align*}
					|S(x) - S_n(x)| &= \left|\sum_{k = n + 1}^{\infty} \frac{x^k}{k!} \right| \\
				...
				\[
					\lim_{n \to \infty} \sum_{k = n + 1}^{\infty} \frac{|a|^k}{k!} = 0
				\]
				Now $\sum_{k = n + 1}^{\infty} \frac{|a|^k}{k!}$ is an upperbound for $|S(x) - S_n(x)|$ and $\sup_{x \in D} |S(x) - S_n(x)|$ is the smallest upperbound, so:
				\[
					\sup_{x \in D} |S(x) - S_n(x)| = \| S - S_n \|_\infty \le \sum_{k = n+1}^{\infty} \frac{|a|^k}{k!} \xrightarrow{n \to \infty} 0
				\]
        ...
        \begin{parts}
        	...
        	\[
	        	\sum \left|\frac{x^k}{k^2}\right| = \sum \frac{x^k}{k^2} \leq \sum \frac{1}{k^2}\,.
        	\]
        	But $\sum 1/k^2$ is a harmonic series with $2 > 1$, hence it is convergent and by Weierstrass' M-test we have uniform convergence of the original series.
        	...
        	\]
        	But then $f_k'(x) = 0$ if $x^{k-1} = x^k$ and therefore $x = 0 \vee x = 1$. For $x=0$ the function sequence is zero, hence its maximum is attained at $x=1$ and therefore
        	\[
        	...
        	\[
	        	\sum \left|\frac{k^r}{e^{kx}}\right| \leq \sum \frac{k^r}{e^{ka}}\,.
        	\]
		...
		\begin{parts}
			...
			\[
				f(x) = \frac{d}{dx}\frac{1}{1-2x} = \sum 2k(2x)^{k-1}
			\]
			This only holds if $|x| < 1/2$.
